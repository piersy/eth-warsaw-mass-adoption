import thresholdBls from "@celo/blind-threshold-bls";
...
export class WebBlsBlindingClient implements BlsBlindingClient {
  ...
  async unblindAndVerifyMessage(base64BlindSig: string): Promise<string> {
    ...
    const unblindMessage = thresholdBls.unblind(blindedSignature, this.blindedValue.blindingFactor);
    // this throws on error
    thresholdBls.verify(this.odisPubKey, this.rawMessage, unblindMessage);
